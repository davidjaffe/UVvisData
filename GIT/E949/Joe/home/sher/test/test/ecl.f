...
c 14 Jan 1999   Compute expected limits more efficiently from the pdf functions
c               of the test statistic rather than with a toy MC
c 22 Jan 1999   Fix a bug introduced on 14 Jan 1999 in e_avcls_f
c 16 Dec 1999   Add an s95 calculator
c 20 Jun 2001   Add in expected s95 calculations
...
c d: observed data candidate count (integer, of dimension n)
c OUTPUTS:
...
c check to see if we are done -- make sure we err on the
c cl<0.05 side (no undercoverage)

          if (cls.lt.0.05.and.cls.gt.0.05-0.001) then
            return
          endif
          if (abs((sflast/sf) - 1.0).lt.0.001) then
            if (cls.gt.0.05) sf = sfh
            return
          endif
...
c start with a null channel and then combine the others one by one onto it.
      ...
      call e_estlist(0.0D0,0.0D0,accprob,accest,nacc,bgonly)
      do i=1,n
        call e_estlist(s(isort(i)),b(isort(i)),prob,est,nlist,bgonly)
...
c with a smaller estimator as the confidence level (-> upper limit).
...
c find all points in the list with an estimator less than
c the data estimator and report the probability to see an experiment
c with a larger estimator as the confidence level (-> lower limit).
...
c 29 June -- fix problem with the CLb expectation in the denominator
     ...
     >   aclsp95,aclsp68,aclsm,aclsm68,aclsm95,clbm)
      ...
      include 'ecl.inc'
      include 'eclplist.inc'
      real*8 s(*),b(*),aclsp95,aclsp68,aclsm,aclsm68,aclsm95
      integer i,j,n
      real psumb,psumsb
      real clbm
...
c of candidates in the data d (a list for all channels).
      ...
      x = 0.0D0
      do i=1,n
        call e_estimator(s(i),b(i),d(i),estim,xi)
...
c outcomes 
      ...
      implicit none
      include 'ecl.inc'
      real*8 s,b,prob(maxlist),est(maxlist),xi
...
c doesn't really matter to the combiner because all it wants
...
c special case -- expect no events (channel switched off)
      ...
      if (s.le.0.0D0.and.b.le.0.0D0) then
        nlist = 1
        prob(1) = 1.0D0
        est(1) = 0.0D0
        return
...
c won't take us negative.
      ...
      if (cvm.gt.30) then
        xlo = cvm - 5.0*sqrt(cvm)
        xhi = cvm + 5.0*sqrt(cvm)
        dx = (xhi - xlo)/float(30)
        psum = 0.0D0
        do i=1,30
          x = xlo + dx*(i-1)
          call e_poiss(cvm,nint(x),prob(i))
...
c otherwise then do an explicit listing of all possible final outcomes
...
c 13 November TRJ -- added in ALEPH and L3 estimator functions
...
c this one really doesn't work
...
c for exactly zero and one reserved for exaclty one.
     ...
     >    ' Exceeds internal limit: ',maxbin
      ...
      do i=1,n
        if (b(i).lt.0.0D0) then
          write(*,*) 'Negative expected background: ',i,b(i)
        endif
        if (s(i).lt.0.0D0) then
          write(*,*) 'Negative expected signal: ',i,s(i)
        endif
        if (d(i).lt.0) then
          write(*,*) 'Negative candidate count: ',i,d(i)
        endif
        if (b(i).eq.0.0D0.and.d(i).gt.0) then
          write(*,*) 'No BG but have a candidate ',i,b(i),d(i)
        endif
c        if (b(i)+s(i).gt.130.) then
c          write(*,*) 'Very large expected s+b -- caution so far'
c          write(*,*) ' i,s,b: ',i,s(i),b(i)
c        endif
      enddo
...
c s, b, and d at nb.
      ...
      nb = 1
      call e_simadda(s(1),b(1),xmin)
      do i=2,n
        call e_simadda(s(i),b(i),x)
