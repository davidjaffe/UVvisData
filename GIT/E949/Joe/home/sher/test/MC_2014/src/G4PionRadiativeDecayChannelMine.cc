...
//    if (GetVerboseLevel()>0) {
      G4cout << "G4RadiativePionDecayChannelMine:: constructor :";
...
//#endif
  ...
  // new code, implemented by A.Sher 2010.
  d2wmax = 17000.;
...
G4PionRadiativeDecayChannelMine::~G4PionRadiativeDecayChannelMine()
{
}

G4DecayProducts *G4PionRadiativeDecayChannelMine::DecayIt(G4double) 
{

#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) 
                 G4cout << "G4PionRadiativeDecayChannelMine::DecayIt ";
#endif
  ...
  do {
      ...
      // new code
      //       y = 2.01*beta+ G4UniformRand()*(0.995-2.01*beta);
      y = 2.0*beta+ G4UniformRand()*(1.+beta*beta-2.0*beta);
      xl = 1.-0.5*(y+std::sqrt(y*y-4.*beta*beta));
      xu = 1.-0.5*(y-std::sqrt(y*y-4.*beta*beta));
